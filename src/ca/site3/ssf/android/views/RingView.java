...
public class RingView extends SurfaceView {
	...
	public float playerActionPoints[] = { (float) 0, (float) 0 };
	...
	public void createEmitters() {
		...
		for (int i = 0; i < num_effects_in_ring; i++) {
			Emitter newEmitter = new Emitter(
					(int) (ringD * Math.cos((-i + ring_offset) * angle)),
					(int) (ringD * Math.sin((-i + ring_offset) * angle)));
		...
		for (int i = -1 * (num_row_effect_length / 2); i < num_row_effect_length / 2; i++) {
			Emitter new_ring_flame = new Emitter(-i * rows_x_offset
					- (rows_x_offset / 2), -1 * rows_y_offset);
			new_ring_flame.location = Location.RIGHT_RAIL;
			new_ring_flame.locationIndex = index;
			emitters.add(new_ring_flame);
			new_ring_flame = new Emitter(-i * rows_x_offset - (rows_x_offset / 2),
					rows_y_offset);
	...
	public static FireEmitter.Location getEmitterLocation(int index) {
		...
		if (index % 2 == 0)
			return FireEmitter.Location.LEFT_RAIL;
	...
	public void setupPaints() {
		...
		paintHealthBackground.setColor(getResources()
				.getColor(R.color.healthbar_bg));
		...
		paintActionPoints = new Paint();
		paintActionPoints.setColor(getResources().getColor(R.color.action_points));
		paintActionPoints.setAntiAlias(true);
	...
	private void drawHealthbars(Canvas canvas) {
	  ...
	  // draw the player 1 actionpoints background
		path = new Path();
		path.moveTo(0, actionBarYOffset);
		path.lineTo(healthbarWidth, actionBarYOffset);
		path.lineTo(healthbarWidth, actionBarYOffset
				+ otherbarHeight);
		path.lineTo(0, actionBarYOffset + otherbarHeight);
		canvas.drawPath(path, paintOtherBackground);
		
		path = new Path();
		path.moveTo(0, actionBarYOffset);
		path.lineTo(healthbarWidth * playerActionPoints[1] / 100, actionBarYOffset);
		path.lineTo(healthbarWidth * playerActionPoints[1] / 100, actionBarYOffset
				+ otherbarHeight);
		path.lineTo(0, actionBarYOffset + otherbarHeight);
		canvas.drawPath(path, paintActionPoints);
		...
		path = new Path();
		path.moveTo(getWidth(), healthbarHeigth);
		path.lineTo(getWidth() - healthbarWidth * playerActionPoints[0] / 100, healthbarHeigth);
		path.lineTo(getWidth() - healthbarWidth * playerActionPoints[0] / 100, healthbarHeigth
				+ otherbarHeight);
		path.lineTo(getWidth(), healthbarHeigth + otherbarHeight);
		canvas.drawPath(path, paintActionPoints);
		...
		path.lineTo(getWidth() - healthbarWidth * playerHealth[0] / 100,
				healthbarHeigth);
		path.lineTo(getWidth(), healthbarHeigth);
		canvas.drawPath(path, paintHealth);
	...
	public boolean onTouchEvent(MotionEvent event) {
		if (!isInDrawMode)
			return true; // touching an emitter does nothing
	...
	private void touch_move(float x, float y) {
		...
		for (Emitter emitter : emitters) {
			if (Math.sqrt(square(x - emitter.x - ringX)
					+ square(y - emitter.y - ringY)) < emitterTouchRadius) {
				// FIXME the touch should be sending an event to the server, not
				// changing how the ring is drawn directly
				emitter.touching = true;
				
				if (onEmitterTouch != null) {
					onEmitterTouch.onEmitterTouch(emitter, emitter.location,
							emitter.locationIndex);
				}
			}
		}
	}
	...
	public void handleFireEmitterEvent(FireEmitterChangedEvent event) {
		...
		if (targetEmitter != null) {
			...
			for (Entity entity : event.getContributingEntities()) {
				targetEmitter.intensity = Math.max(targetEmitter.intensity,
	...
	public abstract class OnEmitterTouch {
		public abstract void onEmitterTouch(Emitter emitter,
				FireEmitter.Location location, int index);
	}
}